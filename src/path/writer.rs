use {Path, PathCommand, PathSegment, WriteBuffer, WriteOptions};
    ...
    if opt.use_implicit_lineto_commands {
        let check_implicit = || {
            if let Some(ref pcmd) = *prev_cmd {
                ...
                } else if pcmd.cmd == PathCommand::MoveTo && seg.cmd() == PathCommand::LineTo {
...
mod tests {
    ...
    fn write_19() {
        ...
        assert_eq!(
            path.with_write_opt(&opt).to_string(),
            "m10 20A10 10 0 1 0 0 0 2 2 0 1 0 2 0"
        );
    }
